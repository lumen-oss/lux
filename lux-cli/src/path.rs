        ...
        return match cmd {
            PathCmd::Full(args) => {
                ...
                if !args.no_bin {
                    ...
                    if !path.is_empty() {
                        result.push_str(format_export(&args.shell, "PATH", &path).as_str());
                        result.push('\n');
                    }
                }
            ...
            PathCmd::Bin => {
                let paths = Paths::new(&tree)?;
                println!("{}", mk_bin_path(&paths, prepend)?);
                Ok(())
            }
            PathCmd::Lua | PathCmd::C | PathCmd::Init => Err(miette!(
                help = "Luau uses the `.luaurc` (generated by Lux) for module resolution",
                "`lx path {}` is not supported under Luau",
                cmd.name()
            )),
        };
    }
